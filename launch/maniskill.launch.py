...
def launch_setup(context):
    ...
    bridge = create_bridge_node(
        ...
        python=LaunchConfiguration("maniskill_python"),
        viewer=LaunchConfiguration("maniskill_viewer"),
    ...
    rsp = Node(
        ...
        output="screen",
        parameters=[resources.robot_description, {"use_sim_time": True}],
    )
    move_group = Node(
        ...
        + [
            {
                ...
                "trajectory_execution.allowed_execution_duration_scaling": ParameterValue(
                    LaunchConfiguration("trajectory_execution_duration_scaling"),
                    value_type=float,
                ),
                "trajectory_execution.allowed_goal_duration_margin": ParameterValue(
                    LaunchConfiguration("trajectory_execution_goal_margin"),
                    value_type=float,
                ),
    ...
    rviz_config = (
        ...
        or scene_spec(scene).rviz_config
        or os.path.join(share, "rviz/dual_fr3.rviz")
    )
    rviz = Node(
        package="rviz2",
        executable="rviz2",
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "cable_solver", default_value="mpm", choices=CABLE_SOLVERS
            ),
            DeclareLaunchArgument(
                "insertion_enabled", default_value="true", choices=("true", "false")
            ),
            DeclareLaunchArgument(
                "load_cable",
                default_value="true",
            ...
            DeclareLaunchArgument(
                "trajectory_execution_duration_scaling", default_value="10.0"
            ),
            DeclareLaunchArgument(
                "trajectory_execution_goal_margin", default_value="5.0"
            ),
            DeclareLaunchArgument(
                "capabilities", default_value="move_group/ExecuteTaskSolutionCapability"
            ),
